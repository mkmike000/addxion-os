---
type: fundamental
title: Branding
description: Markenstrategie ADDXION. Eine Datei. Herkunft MASTERPLAN addxion-com 2026-08-24.
status: active
owner: shared
updated: 2026-09-03
tags: [branding]
sources:
  - id: com-masterplan
    resource: addxion-com/MASTERPLAN.md
    title: Masterplan addxion.com (Stand 2026-08-24)
  - id: docs-branding
    resource: addxion-docs/src/content/docs/branding/
    title: Branding (ältere Starlight-Fassung; Story/Archetype)
  - id: zwei-kanaele
    resource: wiki/decisions/zwei-kanaele.md
    title: Öffentliche Kanäle this.type und addxion.ai
---

# Ein Satz

„Wir fügen Intelligenz hinzu – dorthin, wo sie im Business wirklich wirkt.“

# Auftrag

ADDXION ist Fullstack-Partner für das KI-Zeitalter: Marke, Web, Shop und Software aus einer Hand.

Zwei Verkaufsschienen, klar getrennt:

| Art | Was | Wo |
| --- | --- | --- |
| Produkt | Software, die Kundinnen selbst nutzen: AI, Automationen, Inspiration | App unter addxion.ai; Produktseiten auf addxion.com |
| Dienstleistung | Umsetzung durch uns: Content, Werbung, Websites, Shops, Apps | Startseite und Leistungsseiten |

Die Startseite ist der Einstieg für Website, Fotos und Videos, Shop, App oder Kampagnenflächen, und hält den Weg in die eigenen Produkte offen.

# Öffentliche Stimmen

Firma spricht auf addxion.ai (Labor, Muster, Branchen). Person spricht auf this.type (Ort, Urteil, Auftrag). Regel: [Zwei Kanäle](decisions/zwei-kanaele.md), Ablauf: [Content Zwei Kanäle](processes/content-zwei-kanaele.md).

# Zielgruppe

**Primär:** ambitionierte KMU ohne eigene Tech-Abteilung, die keine generischen No-Code-Lösungen wollen.

**Sekundär:** Gründer/GF (Fahrschulen, Handwerk, E-Commerce); Agenturen, kein White-Label in v1; Teams, die Tempo und Kontrolle wollen.

**Ton:** konkret. Keine KI-Floskeln, keine Gedankenstriche in Fließtext. HTML-Title: Nutzen, kein Produktname, kein ADDXION.

# Story

Herkunft: Unzufriedenheit mit Websites, die isoliert daliegen. Unternehmen brauchen nicht mehr Tools, sondern Intelligenz in bestehenden Prozessen.

# Archetype

Primär Magier (Transformation, greifbare Produktivität). Beimischung Creator (bauen, Neon intern, Custom). Keine leeren Zaubertricks; kein Handwerk ohne Wirkung.

# Vision

Inhalt folgt.

# Mission

Inhalt folgt. Kandidat intern (Notizbuch 2026-09-03): Menschen durch KI-Systeme in Selbständigkeit leiten. Öffentlich nicht verwenden, bis gesetzt.

# Werte

Inhalt folgt.

# Leistungen

Reihenfolge in der Übersicht: Content, Werbung, Websites, Shops, Apps, Automationen.

1. **Content:** Fotos und Videos für Site, Shop und Kanäle. Social ist Format (Feed, Story, Reel), nicht Kanalbetrieb. Companion zu Websites, Shops und Werbung, kein Unterpunkt.
2. **Werbung:** Landingpages, Tracking, Conversion, Kampagnen auf der eigenen Fläche. Motiv kommt aus Content. Kein eigenes Design, sondern Passung. Kein reines Media-Buying als Kernangebot.
3. **Websites:** individuelle Sites, Marke und Performance.
4. **Shops:** Shopify von Null oder Relaunch. Unterkategorien: Einrichtung oder Umgestaltung, Theme, App-Integrationen, Fehlerbehebung, Audit, Produkte und Kollektionen. Shopify Partner ist Vertrauensanker, keine eigene Produktlinie.
5. **Apps:** Web-Apps, Portale, APIs, wenn Theme und Plugin nicht reichen.
6. **Automationen:** Produkt für laufende Workflows. Shop-Integrationen und Custom-Apps sind Dienstleistung, wenn Standard-Apps nicht reichen.

**Inspiration** (Produkt): in Minuten sehen, wie Content, Werbung, Website, Shop und App zusammengehören. Strategie über alle Bausteine; jede Leistung eine Taktik darunter. Kein Marken-Lookbook, kein Website-Scan. Optionaler Companion zu allen Leistungen. Formular `/inspiration/`.

# Neon

Internes Design-System, Code `addxion-neon`. Kein öffentliches Produkt auf addxion.com (`/neon/` 301 auf Start). Partner-Badges nur an Dienstleistungen.

# Was wir nicht tun

- Keine zweite Designsprache, keine 3D-Service-Cards. Hairline- und Section-Grammatik.
- Keine Plattform-Docs auf addxion.com.
- Kein dritter Einstieg „Social Media“. Kein Community-Management als Kernangebot.
- Content nicht unter Websites oder Shops hängen.
